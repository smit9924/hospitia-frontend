...
export class CreateAdmin implements OnInit {
  ...
  protected readonly titleText = computed(() =>
    this.isEditMode() ? $localize`Edit Admin` : $localize`Create Admin`,
  );
  ...
  protected readonly adminForm = this.createAdminForm();
  ...
  private createAdminForm(): AdminForm {
  ...
  private loadAdmin(guid: string): void {
  ...
  private createAdmin(): void {
    const payload: UserCreateRequest = this.adminForm.getRawValue();
      ...
      .subscribe({
        next: () => {
          this.showSuccessSnackbar(
            $localize`Admin created successfully. A welcome email has been sent.`,
          );
          void this.router.navigateByUrl(appRoutes.adminListing);
        },
  ...
  private confirmAndSave(): void {
    this.dialogService
      ...
      .subscribe({
        next: () => {
          this.showSuccessSnackbar($localize`Admin updated successfully.`);
          void this.router.navigateByUrl(appRoutes.adminListing);
        },
        error: (error: HttpErrorResponse) => this.handleSubmitError(error),
      });
