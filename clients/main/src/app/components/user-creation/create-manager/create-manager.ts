...
@Component({
  ...
  styleUrl: './create-manager.scss',
})
export class CreateManager implements OnInit {
  ...
  protected readonly titleText = computed(() =>
    this.isEditMode() ? $localize`Edit Manager` : $localize`Create Manager`,
  );
  ...
  protected readonly managerForm = this.createManagerForm();
  ...
  private createManagerForm(): ManagerForm {
  ...
  private createUser(): void {
    const payload: UserCreateRequest = this.managerForm.getRawValue();
      ...
      .subscribe({
        next: () => {
          this.showSuccessSnackbar(
            $localize`Manager created successfully. A welcome email has been sent.`,
          );
          void this.router.navigateByUrl(appRoutes.managerListing);
        },
    ...
    this.dialogService
      ...
      .pipe(
        ...
        switchMap(() => {
          ...
          const raw = this.managerForm.getRawValue();
      ...
      .subscribe({
        next: () => {
          this.showSuccessSnackbar($localize`Manager updated successfully.`);
          void this.router.navigateByUrl(appRoutes.managerListing);
        },
        error: (error: HttpErrorResponse) => this.handleSubmitError(error),
      });
